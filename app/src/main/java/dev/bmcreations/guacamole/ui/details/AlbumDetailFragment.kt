...
class AlbumDetailFragment: Fragment(), AnkoLogger {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        albumId?.let { vm?.getLibraryAlbumById(it) }
        observe()
    }
    ...
    private fun loadAlbumArt(url: String?) {
        picasso {
            ...
            it.load(url).error(R.drawable.ic_music_fail).into(ra_image)
        }
    }

    private fun observe() {
        vm?.selectedAlbum?.observe(this, Observer { a ->
            a?.let { album ->
                ...
                album.attributes?.trackCount?.let { tracks ->
                    album.relationships?.tracks?.durationInMinutes?.let { length ->
                        context?.let { ctx ->
                            album_duration.text = ctx.formattedStrings[R.string.album_length](tracks.toString(), length.toString())
                        }
                    }
                }
